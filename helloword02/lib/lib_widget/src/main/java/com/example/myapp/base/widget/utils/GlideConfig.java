...
//                .placeholder(R.drawable.base_default_img_square)
                // 设置默认加载出错占位图
//                .error(R.drawable.base_default_img_square));
    }

    @Override
    public void registerComponents(@NonNull Context context, @NonNull Glide glide, @NonNull Registry registry) {
        // Glide 默认使用的是 HttpURLConnection 来做网络请求，这里切换成更高效的 OkHttp
//        registry.replace(GlideUrl.class, InputStream.class, new OkHttpLoader.Factory((Call.Factory) RetrofitCreateHelper.getInstance().initOkHttp()));
    }
