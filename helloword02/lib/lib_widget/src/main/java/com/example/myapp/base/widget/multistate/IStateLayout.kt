...
internal interface IStateLayout {
     ...
     */
    val isContent
        get() = currentState == LayoutStateType.STATE_CONTENT

    /**
     * 当前是否是加载中视图
     */
    val isLoading
        get() = currentState == LayoutStateType.STATE_LOADING

    /**
     * 当前是否是空数据视图
     */
    val isEmpty: Boolean
        get() = currentState == LayoutStateType.STATE_EMPTY

    /**
     * 当前是否是错误视图
     */
    val isError: Boolean
        get() = currentState == LayoutStateType.STATE_ERROR

    /**
     * 当前是否是断网视图
     */
    val isNoNetwork: Boolean
        get() = currentState == LayoutStateType.STATE_NO_NETWORK
    ...
    fun ViewGroup.showLoadingView(
        ...
        if (loadingView == null) {
            ...
            loadingView!!.tag = LayoutStateType.STATE_LOADING
            viewTags.add(LayoutStateType.STATE_LOADING)
        ...
        showViewByState(LayoutStateType.STATE_LOADING)
    }
    ...
    fun ViewGroup.showEmptyView(
        ...
        if (emptyView == null) {
            ...
            emptyView!!.tag = LayoutStateType.STATE_EMPTY
            viewTags.add(LayoutStateType.STATE_EMPTY)
        ...
        emptyView!!.setOnChildViewClickListener(
            ...
            if (clickViewIds.isEmpty()) emptyInfo.clickViewIds else clickViewIds.toList()
        )
        //设置提示文字
        if (hintTextId.isLegalResId && hintText != null) {
            emptyView!!.findViewById<TextView>(hintTextId).text = hintText
        }
        showViewByState(LayoutStateType.STATE_EMPTY)
    }

    fun ViewGroup.showContentView() {
        // 移除非STATE_CONTENT视图
        children.forEach {
            if (it.tag != LayoutStateType.STATE_CONTENT && it.tag in viewTags) {
                removeView(it)
            }
        }
        // 显示内容
        children.first { it.tag == LayoutStateType.STATE_CONTENT || it.tag !in viewTags }.visibility =
            View.VISIBLE
        changeViewStatus(LayoutStateType.STATE_CONTENT)
    }
    ...
    fun ViewGroup.showErrorView(
        ...
        if (errorView == null) {
            ...
            errorView!!.tag = LayoutStateType.STATE_ERROR
            viewTags.add(LayoutStateType.STATE_ERROR)
        ...
        errorView!!.setOnChildViewClickListener(
            ...
            if (clickViewIds.isEmpty()) errorInfo.clickViewIds else clickViewIds.toList()
        )
        //设置提示文字
        if (hintTextId.isLegalResId && hintText != null) {
            errorView!!.findViewById<TextView>(hintTextId).text = hintText
        }
        showViewByState(LayoutStateType.STATE_ERROR)
    }
    ...
    fun ViewGroup.showNoNetworkView(
        ...
        if (noNetworkView == null) {
            ...
            noNetworkView!!.tag = LayoutStateType.STATE_NO_NETWORK
            viewTags.add(LayoutStateType.STATE_NO_NETWORK)
        ...
        noNetworkView!!.setOnChildViewClickListener(
            ...
            if (clickViewIds.isEmpty()) noNetworkInfo.clickViewIds else clickViewIds.toList()
        )
        //设置提示文字
        if (hintTextId.isLegalResId && hintText != null) {
            noNetworkView!!.findViewById<TextView>(hintTextId).text = hintText
        }
        showViewByState(LayoutStateType.STATE_NO_NETWORK)
    }
