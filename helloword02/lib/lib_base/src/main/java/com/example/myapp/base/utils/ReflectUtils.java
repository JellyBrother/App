...
public final class ReflectUtils {
    ...
    public static Class<?> getGenericSuperclass(Class<?> object) {
        Type superClass = object.getGenericSuperclass();
        if (superClass instanceof ParameterizedType) {
            ParameterizedType parameterizedType = (ParameterizedType) superClass;
            Type actualTypeArgument = parameterizedType.getActualTypeArguments()[0];
            return getRawType(actualTypeArgument);
        } else {
            String className = object == null ? "null" : object.getName();
            throw new IllegalArgumentException("getGenericSuperclass Expected a Class, ParameterizedType, or GenericArrayType, but <" + object + "> is of type " + className);
        }
    }
    ...
    public static Class<?> getRawType(Type type) {
        if (type instanceof Class) {
            ...
            throw new IllegalArgumentException("getRawType Expected a Class, ParameterizedType, or GenericArrayType, but <" + type + "> is of type " + className);
        }
    }
    ...
    public static List<String> getClassFieldNames(Class clazz) {
        List<String> fieldNames = getClassFieldNames(clazz, null);
        return fieldNames;
    }

    private static List<String> getClassFieldNames(Class clazz, List<String> fieldNames) {
        if (fieldNames == null) {
        ...
        try {
            ...
            if (!ListUtil.isEmpty(fields)) {
                for (Field field : fields) {
                    try {
                        Object aChar = field.get(clazz);
                        fieldNames.add(String.valueOf(aChar));
                    } catch (Throwable t) {
                        LogUtils.e("classHasField  Throwable1:" + t);
                    }
                }
            }
            Class[] innerClazz = clazz.getDeclaredClasses();
            if (!ListUtil.isEmpty(innerClazz)) {
                for (Class cls : innerClazz) {
                    getClassFieldNames(cls, fieldNames);
                }
            }
        } catch (Throwable t) {
            LogUtils.e("classHasField  Throwable2:" + t);
        }
        return fieldNames;
    }
     ...
     * @return true包含
     */
    public static boolean classHasFieldName(Class clazz, String value) {
        List<String> classFieldNames = getClassFieldNames(clazz);
        return classFieldNames.contains(value);
    }
}
