...
public final class KeyboardUtils {
    ...
    private static int getDecorViewInvisibleHeight(@NonNull final Window window) {
        ...
        if (delta <= ImmersionBar.getNavigationBarHeight(decorView.getContext()) + ImmersionBar.getStatusBarHeight(decorView.getContext())) {
    ...
    private static int getContentViewInvisibleHeight(final Window window) {
        ...
        LogUtils.d("KeyboardUtils", "getContentViewInvisibleHeight: " + (contentView.getBottom() - outRect.bottom));
        int delta = Math.abs(contentView.getBottom() - outRect.bottom);
        if (delta <= ImmersionBar.getNavigationBarHeight(contentView.getContext()) + ImmersionBar.getStatusBarHeight(contentView.getContext())) {
            return 0;
        }
        return delta;
    }
