...
public final class DeviceUtils {
    ...
    private static final Object sUdidLock = new Object();
    public volatile static String randomId = null;
...
//        if (checkDebuggerConnected) return true;
    ...
    public static float getSizeInDp(Configuration configuration) {
    ...
    public static String getRandomId() {
        if (randomId == null) {
            synchronized (sRandomIdLock) {
                randomId = SPUtils.getInstance(DEVICE).getString(RANDOM_ID);
                if (TextUtils.isEmpty(randomId)) {
                    String id = getManufacturer() + "-" + getModel() + "-" + getSDKVersionName() + "-" + System.nanoTime() + "-";
                    // 随机数
                    Random random = new Random();
                    for (int i = 0; i < 8; i++) {
                        randomId = id + random.nextInt();
                    }
                    SPUtils.getInstance(DEVICE).put(RANDOM_ID, randomId);
                }
                return randomId;
            }
        }
