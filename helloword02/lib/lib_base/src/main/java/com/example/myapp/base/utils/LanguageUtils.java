...
public class LanguageUtils {
    ...
    private static void applyLanguageReal(final Locale locale,
        ...
        Locale destLocal = locale == null ? getLocal(Utils.getApp().getResources().getConfiguration()) : locale;
    ...
    public static Locale getSystemLanguage() {
        return getLocal(Utils.getApp().getResources().getConfiguration());
    }
    ...
    static void pollCheckAppContextLocal(final Locale destLocale, final int index, final Utils.Consumer<Boolean> consumer) {
        ...
        if (isSameLocale(appLocal, destLocale)) {
            ...
            LogUtils.e("LanguageUtils", "appLocal didn't update.");
            consumer.accept(false);
        }
    }

    /**
     * If applyLanguage not work, try to call it in {@see Activity#attachBaseContext(Context)}.
     *
    ...
    static void applyLanguage(final Activity activity) {
        ...
        if (VALUE_FOLLOW_SYSTEM.equals(spLocale)) {
            destLocal = getLocal(Utils.getApp().getResources().getConfiguration());
        } else {
    ...
    private static Locale string2Locale(String str) {
        ...
        if (locale == null) {
            LogUtils.e("LanguageUtils", "The string of " + str + " is not in the correct format.");
