 ...
 */
public class SafeFixedExecutor {

    private static final String TAG = "SafeFixedExecutor";

    protected ExecutorService mExeService;

    protected static HashMap<String, ExecutorService> mServiceMap = new HashMap<>();

    protected SafeFixedExecutor() {
    }

    public SafeFixedExecutor(String moduleName, int number) {
        if (mServiceMap.containsKey(moduleName)) {
            mExeService = mServiceMap.get(moduleName);
        } else {
            ThreadFactory threadFactory = new SafeThreadFactory(moduleName + "FixedThread");
            mExeService = Executors.newFixedThreadPool(number, threadFactory);
    ...
    public <Result> Result syncExecute(final Callable<Result> callable, long timeout, String tag) {
        Future<Result> future = mExeService.submit(callable);
        Result result = null;
        ...
        try {
            ...
            LogUtil.getInstance().e(TAG, "syncExecute " + tag + "  error" + e.toString());
        }
        return result;
    }
