...
import com.jelly.wxtool.message.utils.AbstractTFUtil;

import java.security.InvalidParameterException;
import java.util.ArrayList;
import java.util.List;

public class AddFriendService3 extends AccessibilityService {
    private final String TAG = getClass().getName();

    public static AddFriendService3 mService;

    //初始化
    @Override
    protected void onServiceConnected() {
        super.onServiceConnected();
        ToastUtil.makeText("O(∩_∩)O~~\r\n红包锁定中...");
        mService = this;
    }
    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        AccessibilityNodeInfo biaoQingInfo = findFirst(AbstractTFUtil.newContentDescription("表情", true));
        if (biaoQingInfo != null) {
            ToastUtil.makeText("找到wx的表情图标");//第一次运行可能会吐不出来
     ...
     */
    @Nullable
    public AccessibilityNodeInfo findFirst(@NonNull AbstractTFUtil... tfs) {
        ...
        for (int i = 0; i < tfs.length; i++) {
            if (tfs[i] instanceof AbstractTFUtil.IdTextTF) {
                idTextTFCount++;
                idTextIndex = i;
            }
        }
        switch (idTextTFCount) {
            ...
            case 1://id或text数量为1，先查出对应的id或text，然后再查其他条件
                if (tfs.length == 1) {
                    AccessibilityNodeInfo returnInfo2 = ((AbstractTFUtil.IdTextTF) tfs[idTextIndex]).findFirst(rootInfo);
                    rootInfo.recycle();
                    return returnInfo2;
                } else {
                    List<AccessibilityNodeInfo> listIdText = ((AbstractTFUtil.IdTextTF) tfs[idTextIndex]).findAll(rootInfo);
                    if (PublicUtil.isEmptyList(listIdText)) {
                        break;
                    }
     ...
     */
    public static AccessibilityNodeInfo findFirstRecursive(AccessibilityNodeInfo parent, @NonNull AbstractTFUtil... tfs) {
     ...
     */
    @NonNull
    public List<AccessibilityNodeInfo> findAll(@NonNull AbstractTFUtil... tfs) {
        if (tfs.length == 0) throw new InvalidParameterException("AbstractTF不允许传空");
     ...
     * @param tfs 由于是递归循环，会忽略IdTF和TextTF
     */
    public static void findAllRecursive(List<AccessibilityNodeInfo> list, AccessibilityNodeInfo parent, @NonNull AbstractTFUtil... tfs) {
    ...
    public static void recycleAccessibilityNodeInfo(List<AccessibilityNodeInfo> listInfo) {
        if (PublicUtil.isEmptyList(listInfo)) return;
